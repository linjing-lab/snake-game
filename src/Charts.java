...
public class Charts extends JFrame{
	...
	Charts(JFrame mainFrame){
		...
		for(int i=0;i<10;i++) {
			score_list[i+1]=String.format("%-15s%-20s%-15s", i+1,userList[i].account,userList[i].maxscore);
		}
		...
		for(int i=0;i<10;i++) {
			time_list[i+1]=String.format("%-15s%-20s%-15s", i+1,userList[i].account,userList[i].time);
		}
